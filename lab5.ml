...
single 8-bit (1-byte) integer. Since one byte represents 2 ** 8 = 256
discrete values, there are over 16.7 million (256 * 256 * 256)
possible colors that can be represented with this method.
...
........................................................................
Exercise 2: Define an algebraic color type that supports either
`Simple` colors (from the `color_label` type you defined previously)
or `RGB` colors, which would incorporate a tuple of values for the
three color channels.  You'll want to use `Simple` and `RGB` as the
value constructors in this new variant type.
......................................................................*)
...
the type to be valid:
  ...
  INVARIANT: The red, green, and blue channels must each be a
  non-negative 8-bit int. Therefore, each channel must be in the range
  [0, 255].
...
* validated_rgb : color -> color 
  ...
  Returns its argument unchanged if it is a valid color, and raises an
  appropriate exception otherwise.

* validate_rgb : color -> unit 

  Returns unit; raises an appropriate exception if its argument is not
  a valid color.
...
the next lab, we use the "valid_" alternative approach.)
